...
| 18 | **coffer_exec: async job pattern for long-running commands** — the synchronous MCP tool call times out client-side (~60s) while the child keeps running, so the caller loses the sanitized stdout/exit_code report and must watch the filesystem instead (observed 2026-07-14 running a ~mins-long scraper). Add `coffer_exec_start` returning a job id immediately + `coffer_exec_status`/`coffer_exec_result` polling; keep the sync tool for short commands. Results still scrubbed and audited. | TODO | 1 day |
